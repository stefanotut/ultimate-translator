...
// PATCH LOCALE: vedi blob-sicuro.js. Le estensioni che sostituiscono il
// costruttore Blob rompevano il caricamento di ogni capitolo XHTML.
import { creaBlob } from './blob-sicuro.js'
...
const deobfuscate = async (key, length, blob) => {
    ...
    return creaBlob([array, blob.slice(length)], { type: blob.type })
}
...
class Loader {
    ...
    async createURL(href, data, type, parent) {
        ...
        const url = URL.createObjectURL(await creaBlob([newData], { type: newType }))
...
${doc.querySelector('parsererror').innerText}`)
    ...
    async getCover() {
        ...
        return cover?.href
            ? creaBlob([await this.loadBlob(cover.href)], { type: cover.mediaType })
            : null
    }
