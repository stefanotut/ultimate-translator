...
function emptyLibraryHTML() {
    return `<div class="empty-state">
        ...
        <div class="empty-features">
            ...
            <div class="empty-feature">${icon('read', 22)}<strong>Leggi e ascolta</strong>Ogni libro si apre nel lettore del portale</div>
        </div>
    </div>`;
}
...
function emptyViewHTML(view) {
    ...
    switch (state.view.type) {
        ...
        case 'translated':
            return `<div class="empty-state is-compact"><div class="empty-title">Nessun libro tradotto</div><p class="empty-text">Le traduzioni si fanno dalla pagina «Traduci» del portale: il libro tradotto compare fra i tuoi libri.</p></div>`;
        default:
            return `<div class="empty-state is-compact">
...
function keyBannerHTML() {
    // Nel portale la libreria e' quella dell'account, non del browser: questo
    // avviso ("salva il codice per aprirla altrove") qui direbbe il falso.
    // Il codice resta in "Codice libreria", per il programma di importazione.
    return '';
...
// Nel portale ogni libro delle cartelle e' anche un libro della libreria di
// sempre: si apre nel suo lettore (lettura, ascolto, appunti).
function readBook(id) {
    const book = state.bookById.get(id);
    if (book && book.read_url) {
        window.location.href = book.read_url;
    } else {
        toast('Il libro si sta ancora collegando al lettore: riprova tra qualche secondo', { type: 'info', duration: 3200 });
    }
}
...
function openBookMenu(id, anchor) {
    ...
    openMenu([
        ...
        { label: 'Leggi', icon: 'read', action: () => readBook(id) },
...
async function openKeyModal() {
    ...
    if (!data) return;
    const link = () => `${location.origin}${location.pathname}?chiave=${data.code.replace(/-/g, '')}`;
    openModal({
        title: 'Codice per importare libri',
        wide: true,
        body: `<p>La libreria è quella del tuo account: su telefono, tablet e computer basta entrare nel portale. Questo codice serve solo al programma che carica una cartella di libri dal Mac («Carica libri nella libreria» / <code>import_books.py</code>).</p>
            ...
            <div class="warn-note">${icon('alert', 18)}<span>Chi ha il codice può caricare libri nella tua libreria: non condividerlo.</span></div>
            <div class="modal-divider"></div>
            <p class="muted-note">Hai condiviso il codice per errore? <button class="link-btn" data-rotate>Genera un nuovo codice</button>: quello vecchio smette di funzionare.</p>`,
        onOpen: (modal) => {
            const root = modal.el;
...
async function handleAction(action, node) {
    ...
    switch (action) {
        ...
        case 'drawer-read': readBook(state.drawerId); break;
