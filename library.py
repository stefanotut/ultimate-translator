...
Flask blueprint for the personal book library: the /libreria page, its REST
API, uploads (single books or whole folders) and background auto-tagging.

Every browser gets its own private library, identified by a secret code kept
in an HttpOnly cookie. The same code opens the library on other devices.
"""
