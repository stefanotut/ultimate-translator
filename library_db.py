...
def _migrate_portal_columns(conn):
    """Columns added for the production portal (owner of a library, link to the
    portal's own book). CREATE TABLE IF NOT EXISTS does not add columns to an
    existing table, so a database created by the plain branch gets them here."""
    for table, column in (('libraries', 'owner'), ('books', 'job_id')):
        columns = {r[1] for r in conn.execute(f'PRAGMA table_info({table})')}
        if column not in columns:
            conn.execute(f'ALTER TABLE {table} ADD COLUMN {column} TEXT')
    conn.execute('CREATE UNIQUE INDEX IF NOT EXISTS idx_libraries_owner ON libraries(owner) '
                 'WHERE owner IS NOT NULL')
    conn.execute('CREATE INDEX IF NOT EXISTS idx_books_job ON books(job_id)')
...
def create_library(owner=None):
    ...
    for _ in range(5):
        ...
        try:
            with _write() as conn:
                conn.execute(
                    'INSERT INTO libraries (id, code, created_at, last_seen_at, owner) VALUES (?, ?, ?, ?, ?)',
                    (library_id, code, now, now, owner),
                )
            return {'id': library_id, 'code': code, 'created_at': now, 'last_seen_at': now, 'owner': owner}
        except sqlite3.IntegrityError:
            if owner:
                existing = library_for_owner(owner, create=False)
                if existing:
                    return existing      # created at the same moment by another request
            continue
    raise LibraryError('Impossibile creare la libreria', 500)


def library_for_owner(owner, create=True):
    """The library of a portal account (one per account), created on first use."""
    owner = (owner or '').strip().lower()
    if not owner:
        return None
    with _read() as conn:
        row = conn.execute('SELECT * FROM libraries WHERE owner = ?', (owner,)).fetchone()
    if row:
        return dict(row)
    return create_library(owner) if create else None


def library_owner(library_id):
    with _read() as conn:
        row = conn.execute('SELECT owner FROM libraries WHERE id = ?', (library_id,)).fetchone()
...
_BOOK_INSERT_FIELDS = {
    ...
    'total_chars', 'estimated_tokens', 'has_cover', 'color', 'favorite', 'job_id',
}
...
def set_book_job(library_id, book_id, job_id):
    with _write() as conn:
        conn.execute('UPDATE books SET job_id = ?, updated_at = ? WHERE id = ? AND library_id = ?',
                     (job_id, time.time(), book_id, library_id))


def job_links(library_id):
    """Every book of the library (trashed ones too) with its portal link and file."""
    with _read() as conn:
        rows = conn.execute(
            'SELECT id, job_id, file_path, deleted_at FROM books WHERE library_id = ?', (library_id,)
        ).fetchall()
    return [dict(r) for r in rows]


def purge_book_now(library_id, book_id):
    """Delete one book record right away (its portal book is already gone)."""
    with _write() as conn:
        files, _ = _purge_books(conn, library_id, [book_id])
...
def _purge_books(conn, library_id, book_ids):
    """Returns (files to remove, portal job ids of the purged books)."""
    files = []
    jobs = []
    for chunk in _chunked(book_ids):
        for r in conn.execute(
            f'SELECT id, file_path, has_cover, job_id FROM books WHERE id IN ({_ph(chunk)})', chunk
        ):
...
def purge_items(library_id, book_ids=(), folder_ids=(), skip_busy=False):
    ...
    with _write() as conn:
        ...
        files, jobs = _purge_books(conn, library_id, book_set)
        for chunk in _chunked(folder_set):
            conn.execute(f'DELETE FROM folders WHERE id IN ({_ph(chunk)})', chunk)
        _delete_orphan_tags(conn, library_id)
    remove_files(files)
    _notify_purged(library_id, jobs)
    return {'books': len(book_set), 'folders': len(folder_set)}


# Called with (library_id, [portal job ids]) after books are deleted for good.
# The production portal sets it: deleting a book for good from the trash of the
# folders page deletes it from the portal's library too.
on_books_purged = None


def _notify_purged(library_id, job_ids):
    if job_ids and on_books_purged:
        try:
            on_books_purged(library_id, job_ids)
        except Exception:
            logger.exception('Could not delete the portal books of purged library books')
